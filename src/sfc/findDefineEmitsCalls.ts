...
export interface DefineEmitsCallSite {
...
function isDefineEmitsIdentifier(node: unknown): node is LocalIdentifier {
  return (
    !!node &&
    typeof node === "object" &&
    (node as LocalIdentifier).type === "Identifier" &&
    (node as LocalIdentifier).name === "defineEmits"
  );
}
...
export function findDefineEmitsCalls(sfc: ParsedSfc): DefineEmitsCallSite[] {
  const calls: DefineEmitsCallSite[] = [];
  ...
  for (const [blockName, block] of [
    ...
    traverse(ast, {
      CallExpression(path: NodePath) {
        ...
        if (!isDefineEmitsIdentifier(node.callee)) return;
        if (path.scope.getBinding("defineEmits")) return;
